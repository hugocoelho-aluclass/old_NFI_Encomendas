...
namespace NVidrosEncomendas.WebServer.BusinessLogic
{
    public class AdminUtilizadoresBL
    {
        ...
        {
            var res = DbContext.Operadores
                .Include(x => x.DepartamentosSav)
                .Where(x => x.UtilizadorId == id).FirstOrDefault();
        ...
        {
            return DbContext.Operadores.Include(x => x.DepartamentosSav).Where(x => !x.Anulado);
        }
        ...
        public void AtualizaOperador(Operadores op)
        {
            try
            {
                Operadores _op = DbContext.Operadores.Include(x => x.DepartamentosSav).Where(x => x.UtilizadorId == op.UtilizadorId).FirstOrDefault();
            ...
            if (operadosAremover.Count != 0)
            {
                foreach (var item in operadosAremover)
                {
                    List<Savs> savs = DbContext.Savs
                        ...
                        .Include(x =>  x.EditadoPor)
                        .Where(x => x.CriadoPor.UtilizadorId == item.UtilizadorId || x.EditadoPor.UtilizadorId == item.UtilizadorId).ToList();
                    if (savs.Count() != 0)
                    {
        ...
        public void GarantirMasterAdmin()
        {
            ...
            {
                _userManager.AddToRole(userGet.Id, "ADMIN");
                //userGet.Roles.Add(new Identity.Include(x =>  x.EditadoPor)UserRole() { RoleId = roleAdmin.Id, UserId = userGet.Id });
            }

            var operadorExiste = OperadoresListaMasterIncluido().Any(x => x.NomeLogin == "masteradmin");
            if (!operadorExiste)
            {
                try
                {
                    DbContext.Operadores.Add(new Operadores()
                    {
                        NomeLogin = "masteradmin",
                        Ativo = true,
                        Admin = true,
                        AdminSav = true,
                        AspIdentityId = user.Id,
                        NomeCompleto = user.UserName,
                        Email = user.Email,
                        Password = "aaaaa",
                        CriadoData = DateTime.Now,
                        EditadoData = DateTime.Now
