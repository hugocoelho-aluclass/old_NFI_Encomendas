...
namespace NfiEncomendas.WebServer.BusinessLogic
{
    public class TipoEncomendasBL
    {
        ...
        {
            return DbContext.TipoEncomendas.Include("SetorEncomenda").AsParallel().OrderBy(x => x.NumTipoEncomenda);
        }

        public IEnumerable<TipoEncomendas> TipoEncomendasLista()
        {
            return DbContext.TipoEncomendas.Include("SetorEncomenda").AsParallel().Where(x => x.Anulado == false).OrderBy(x => x.NumTipoEncomenda);
        }

        public TipoEncomendas LerTipoEncomenda(int id)
        {
            var res = DbContext.TipoEncomendas.Include("SetorEncomenda").Where(x => x.NumTipoEncomenda == id).FirstOrDefault();
            if (res == null)
            {
